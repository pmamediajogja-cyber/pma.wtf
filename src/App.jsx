    ...
    <>
      ...
      <main id="top">
        ...
        <section className="hero section-wrap">
          ...
          <div className="hero-copy reveal">
            ...
            <div className="hero-actions">
              ...
              <a className="button button-ghost" href="/profile">Who is PMA?</a>
            </div>
          </div>
          ...
          <div className="design-toolbar">
            ...
            <div className="design-filters">
              {["ALL", "FREE", "PREMIUM"].map((option) => (
                <button key={option} className={filter === option ? "filter-button active" : "filter-button"} onClick={() => setFilter(option)}>{option}</button>
              ))}
            </div>
          </div>
          <div className="design-grid">{filteredDesigns.map((d) => <DesignCard key={d.id} design={d} />)}</div>
        ...
        <section id="services" className="section-wrap section-block split-section">
          <div><span className="section-index">02 / SERVICES</span><h2>BUILD IT.<br /><em>MAKE IT MATTER.</em></h2></div>
          <div className="service-list">
            {["T-Shirt & Graphic Design", "Brand & Social Media Visuals", "Landing Pages & Websites", "Digital Creative Support"].map((service, index) => (
              <a href="#contact" className="service-row" key={service}><span>0{index + 1}</span><strong>{service}</strong><b>↗</b></a>
            ))}
          </div>
        </section>
