...
function JournalCard({ article, goToJournal }) {
  ...
  const handleKeyDown = (event) => {
    if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openReview(); }
  };
  return (
    <article className="journal-card" role="link" tabIndex={0} aria-label={`Buka review PMA: ${article.title}`} onClick={openReview} onKeyDown={handleKeyDown}>
      <div className="journal-card-visual">
        <img src={media?.image || fallback} alt={media?.credit ? `${article.title} — ${media.credit}` : `${article.title} — PMA editorial image`} loading="lazy" onError={(event) => { if (event.currentTarget.src.endsWith(fallback)) return; event.currentTarget.src = fallback; }} />
        <span className="journal-image-badge">{isOriginal ? "PMA ORIGINAL" : "REAL SOURCE IMAGE"}</span>
      </div>
      ...
      <div className="journal-card-footer">
        ...
        <a href={article.url} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()}>{isOriginal ? "PMA.WTF ↗" : "Source ↗"}</a>
      </div>
    </article>
  );
}
...
function Home({ goToJournal }) {
  ...
  return <>
    <header className="site-header"><a className="brand" href="/" aria-label="PMA.WTF home"><span className="brand-mark">PMA</span><span className="brand-dot">.</span><span>WTF</span></a><button className="menu-toggle" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle menu"><span /><span /></button><nav className={menuOpen ? "nav-links open" : "nav-links"}>{navItems.map(([label, href]) => <a key={label} href={href} onClick={() => setMenuOpen(false)}>{label}</a>)}<a className="nav-cta" href="#contact" onClick={() => setMenuOpen(false)}>Work with me</a></nav></header>
    <main id="top"><div className="home-light-wash" aria-hidden="true" style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 1, background: ["radial-gradient(circle at 72% 16%, rgba(100,232,255,0.10), transparent 28%)", "radial-gradient(circle at 28% 34%, rgba(107,92,255,0.07), transparent 25%)", "linear-gradient(180deg, rgba(255,255,255,0.025), transparent 38%)"].join(","), mixBlendMode: "screen" }} />
      <section className="hero section-wrap"><div className="hero-scroll-square" aria-hidden="true" /><div className="hero-copy reveal"><div className="eyebrow"><span className="status-dot" />DIGITAL CREATIVE / YOGYAKARTA</div><h1>MAKE SOMETHING<br /><em>WORTH CLICKING.</em></h1><p className="hero-lead">Designs, digital products, creative work and ideas — built under <strong>PT PMA Media Yogyakarta.</strong></p><div className="hero-actions"><a className="button button-primary" href="#designs">Explore designs <span>↗</span></a><a className="button button-ghost" href="/profile">Who is PMA?</a></div></div><div className="hero-meta reveal-delay"><div className="meta-line"><span>BASED IN</span><strong>YOGYAKARTA, ID</strong></div><div className="meta-line"><span>FOCUS</span><strong>DESIGN · DIGITAL · TECH</strong></div><div className="meta-line"><span>STATUS</span><strong className="online">OPEN FOR WORK</strong></div></div><div className="scroll-cue">SCROLL <span>↓</span></div></section>
      <section id="designs" className="section-wrap section-block"><div className="section-heading"><div><span className="section-index">01 / DESIGNS</span><h2>THE WORK</h2></div><p>Original graphic work, released as free downloads and premium digital products.</p></div><div className="design-toolbar"><div className="filter-label">FILTER / {filteredDesigns.length.toString().padStart(2, "0")} ITEMS</div><div className="design-filters">{["ALL", "FREE", "PREMIUM"].map((option) => <button key={option} className={filter === option ? "filter-button active" : "filter-button"} onClick={() => setFilter(option)}>{option}</button>)}</div></div><div className="design-grid">{filteredDesigns.map((d) => <DesignCard key={d.id} design={d} />)}</div><div className="section-note">THE LIBRARY WILL GROW / FREE + PREMIUM</div></section>
      <section id="services" className="section-wrap section-block split-section"><div><span className="section-index">02 / SERVICES</span><h2>BUILD IT.<br /><em>MAKE IT MATTER.</em></h2></div><div className="service-list">{["T-Shirt & Graphic Design", "Brand & Social Media Visuals", "Landing Pages & Websites", "Digital Creative Support"].map((service, index) => <a href="#contact" className="service-row" key={service}><span>0{index + 1}</span><strong>{service}</strong><b>↗</b></a>)}</div></section>
      <section id="journal" className="section-wrap section-block journal-section"><div className="section-heading"><div><span className="section-index">03 / JOURNAL</span><h2>JOURNAL</h2></div><p>Cybersecurity, AI, design thinking, technical builds, streetwear culture and PMA's take on what they mean in practice.</p></div><div className="journal-groups">{journalGroups.map((group, groupIndex) => { const articles = allJournalArticles.filter((article) => article.category === group.key); return <section className="journal-group" key={group.key}><div className="journal-group-heading"><div><span className="journal-group-index">{String(groupIndex + 1).padStart(2, "0")} / {articles.length} ARTICLES</span><h3>{group.label}</h3></div><div className="journal-group-tools"><p>{group.note}</p><span className="journal-rail-hint">AUTO-SCROLL / HOVER TO PAUSE</span></div></div><JournalRail articles={articles} goToJournal={goToJournal} /></section>; })}</div><div className="section-note">PMA JOURNAL / EDITORIAL SIGNALS + PMA ORIGINALS</div></section>
      <section id="contact" className="section-wrap contact-section"><span className="section-index">04 / CONTACT</span><h2>HAVE AN IDEA?<br /><em>LET'S BUILD IT.</em></h2><a className="contact-email" href="mailto:pma.media.jogja@gmail.com">pma.media.jogja@gmail.com ↗</a><div className="company-line">PT PMA MEDIA YOGYAKARTA · DIGITAL CREATIVE & TECHNOLOGY</div></section>
    </main>
  </>;
}
...
function App() {
  ...
  useEffect(() => { let title = "PMA.WTF — Digital Creative Hub"; if (path === "/profile") title = "Profile — PMA.WTF"; if (path === "/cv") title = "CV — Imam Falahi"; const journalMatch = path.match(/^\/journal\/(.+)$/); if (journalMatch) { const article = allJournalArticles.find((item) => item.id === journalMatch[1]); if (article) title = `${article.title} — PMA Journal`; } document.title = title; }, [path]);
